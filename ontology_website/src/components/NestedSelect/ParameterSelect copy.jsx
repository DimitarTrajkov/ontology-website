...
import Parameter2Select from "./HyperParameterSelect";
    ...
    const fetchData = async () => {
      try {
        ...
        const updatedData = transformedData.map(item => ({
          ...item,
          picked: chosenCombinations.some(combination => combination.startsWith(`${datasetName}-${modelName}-${item.name}`))
        }));
...
<div>
  {params.map((model, index) => (
    <div key={index}>
      ...
      {model.open && <Parameter2Select datasetName={datasetName} modelName={modelName} paramName={model.name} chosenCombinations={chosenCombinations} setChosenCombinations={setChosenCombinations}/>}
    </div>
  ))}
  <button disabled={!allPicked}>ADD</button>
</div>
